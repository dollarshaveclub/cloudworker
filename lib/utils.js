...
module.exports.extractGlobals = (setgFlagValues) => {
  const globals = {}
  for (const flag of setgFlagValues) {
    if (flag.split("=").length < 2) {
      throw new Error('Invalid global flag format. Expected format of [variable]=[value]')
    }
    const globalComponents = flag.split('=')
    const key = globalComponents[0]
    const value = globalComponents[1]
    if (globals[key]) {
      throw new Error(`Duplicate global variable ${key}`)
    }
    globals[key] = value
  }
}
